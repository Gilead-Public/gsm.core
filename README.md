...
Since {gsm.core} is designed for use in a [GCP](https://en.wikipedia.org/wiki/Good_clinical_practice) framework, we have conducted extensive quality control as part of our development process. Our quality‑control framework includes the following components:
...
-   **Contributor Guidelines** - Step-by-step processes for code development and releases provided in `CONTRIBUTING.md` and required to be followed for contributons.
