...
process. In particular, we rely on several key components to maintain a
consistent and rigorous quality‑control process:

- **Unit Tests** - Unit tests are written for all core functions.
- **Workflow/Qualification Tests** - Additional unit tests confirm that
  core workflows behave as expected. Qualification testing ensures that
  core functions execute as expected on a system-wide scale.
  Qualification includes executing various functional, performance, and
  usability testing. Qualification tests are designed to provide
  developers with a repeatable process that is easy to update and
  document.
- **Code Examples** - The Cookbook articles mentioned above provides a
  series of simple examples, and all functions include examples as part
  of roxygen2 documentation.
- **Code Review** - Code review is conducted using GitHub Pull Requests
  (PRs). The code is reviewed and approved by QC programmer.
- **Function Documentation** - Detailed documentation for each function
  is maintained with roxygen2.
...
- **Continuous Integration** - Continuous integration is provided via
  GitHub Actions. The full list of GitHub Actions utilized can be found
  in the [GitHub Actions
  Workflows](https://gilead-biostats.github.io/gsm.utils/index.html#github-actions-workflows)
  section of the gsm.utils package
- **Regression Testing** - Extensive QC and testing is done before each
  release.

# Qualification Workflow

All quality control checks decsribed above come together into one
qualification workflow supported by the
[`{qcthat}`](https://gilead-biostats.github.io/qcthat/index.html)
package. The `{qcthat}` process provides a structured way to ensure that
all requirements for an R package (documented as GitHub issues) are
properly tested and verified before a package is released. It does so by
compiling the results of all qualification and unit tests that have been
written for the package and comparing them against full set of
documented requirements (associated GitHub issues). This creates a clear
map showing which requirements have evidence supporting their completion
and which still need attention. The process can be run automatically as
part of the release workflow via GitHub Actions, generating a
qualification report that that clearly links requirements, tests, and
outcomes. The report also includes a detailed summary of the computing
environment used to execute both tests and report generation, adding
further traceability. Together, these elements provide a consistent and
transparent quality‑control framework.
...
Refer to our **Contributor Guidelines** that includes step-by-step
processes for code development and releases are provided in
`CONTRIBUTING.md` and visible in the sidebar of the website.
