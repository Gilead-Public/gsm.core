...
###  `Reporting_Metrics`
  ...
  - Inputs: `lWorkflows` - metadata for the corresponding kri(s) made with `workr::MakeWorkflowList()`
...
Assessment workflow metadata objects are passed to the `lWorkflow` parameter in `workr::RunWorkflow()` to define functions and parameters across multiple studies.

The `lWorkflow` object is a named list of metadata and steps defining how each assessment should be run. By default, `workr::MakeWorkflowList()` imports YAML specifications from `workflow/2_metrics` in `{gsm.kri}`. Each item in `lWorkflow` expects the following parameters in the `steps` section:
