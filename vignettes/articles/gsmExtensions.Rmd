...
- `Active`: *optional* A boolean (`true`/`false`) indicating whether the workflow is active. Workflows with `Active: false` are filtered out of workflow lists. If missing or `true`, the workflow is included.
...
- `GenerateRiskSignal`: *optional* A boolean (`true`/`false`) indicating whether a risk signal should be generated when this metric produces a flag. If missing or `true`, a risk signal is generated. Only applicable to metric workflows. We use this flag in our internal systems, and provide it for you to integrate metrics into your own workflows.
...
**Note**: It is important to note that the default behavior of `workr::RunWorkflow()` and `workr::RunWorkflows()` is to return the *last* output in the steps section of the workflow. Therefore, each yaml file, regardless of which directory it is in, should only produce one output, whether that be a data table, list, html output, deployed shiny app, or any other object needed to produce the module output.

The `steps` section is the most complex part of the module configuration and will vary greatly depending on the module type and the specific requirements of the module. `{workr}` provides the standard workflow-runtime functions used to execute module yaml files, while `{gsm.core}` provides many of the analytical functions that those steps call. See `?workr::RunWorkflow()` for more details.

### `steps[]$params` Specification

After processing the YAML `meta` and `spec` sections, `workr::RunWorkflow()` calls `workr::RunStep()` for each step in the `steps` section of the YAML. The `params` section of each step is passed to `workr::RunStep()` as a list of parameters along with a copy of the metadata header (`lMeta`) and any data (`lData`). `workr::RunStep()` then parses the list of `params` by passing data from `lMeta` and `lData` when appropriate. Finally, the parsed parameters are passed to the function specified in the `name` field of the step.
...
In the example below, the steps to produce the AE analysis output is specified. Here, `Threshold`, `GroupLevel`, `Type`, `AccrualThreshold` and `AccrualMetric` are specified in the `meta` section of the workflow, and would be accessed via the `paramVal` process discussed above. As a default, the output of these steps as run with `workr::RunWorkflows()` would be a list of data tables, as specified in the final `list` step of the workflow.
